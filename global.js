...
// Fix Header Split (2 left / 2 right)

(()=>{const d=document,desk=()=>matchMedia&&matchMedia("(hover:hover) and (pointer:fine)").matches;
const norm=s=>(s||"").toLowerCase().replace(/\s+/g," ").trim();
const orderKey=li=>{
  const a=li&&li.querySelector&&li.querySelector(":scope>a");
  const t=norm(a&&a.textContent);
  if(t.includes("studio"))return 1;
  if(t.includes("gorge"))return 2;
  if(t.includes("property"))return 3;
  if(t.includes("connect"))return 4;
  return 99;
};
const fix=()=>{
  if(!desk()) return;
  const hc=d.querySelector(".md-header.layout4 .header-container"); if(!hc) return;
  const L=hc.querySelector(":scope>.left-menu"),R=hc.querySelector(":scope>.right-menu"); if(!L||!R) return;

  // grab ONLY top-level parent items from both sides
  let items=[...L.querySelectorAll(":scope>.menu-item.has-child"),...R.querySelectorAll(":scope>.menu-item.has-child")];

  // if Lofty temporarily duplicates/hides, normalize by unique key (text)
  const seen=new Set(); 
  items=items.filter(li=>{
    const a=li.querySelector(":scope>a"); const k=norm(a&&a.textContent);
    if(!k||seen.has(k)) return false; seen.add(k); return true;
  });

  // sort into the intended order: Studio, Gorge, Property, Connect
  items.sort((a,b)=>orderKey(a)-orderKey(b));

  // enforce 2 left / 2 right
  const left=items.slice(0,2), right=items.slice(2,4);
  left.forEach(li=>L.appendChild(li));
  right.forEach(li=>R.insertBefore(li, R.firstChild)); // keep them before other right-side stuff
};

fix();

// SPA-safe re-arm for ~2000ms
let mo=new MutationObserver(fix);
mo.observe(d.documentElement,{childList:true,subtree:true});
setTimeout(()=>mo.disconnect(),2000);
addEventListener("pageshow",fix,{passive:true});
d.addEventListener("visibilitychange",()=>{if(!d.hidden)fix()},{passive:true});
})();
